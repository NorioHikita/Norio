"""
コンサルティング分析システム + AIイノベーション推進室チーム構成支援
...
# ─── コンサルティングチャット（ストリーミング） ───────────────────────────────

@app.post("/api/chat")
async def chat(request: ChatRequest):
    """フェーズ対応コンサルティングチャット（SSEストリーム）"""
...
# ─── AIチーム構成 マルチエージェントチャット（ストリーミング） ──────────────────

@app.post("/api/ai-team/chat")
async def ai_team_chat(request: AITeamChatRequest):
    """
    AIイノベーション推進室チーム構成支援 - マルチエージェントチャット（SSEストリーム）
    ...
    return StreamingResponse(
        stream_team_agent(api_key, messages),
        media_type="text/event-stream",
        headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
    )


@app.get("/api/ai-team/agents")
